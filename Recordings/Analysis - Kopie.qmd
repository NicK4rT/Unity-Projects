...
dataRB <- dataRB %>%
  rename_at(17, ~'xrotcsv') %>%
  rename_at(18, ~'yrotcsv') %>%
  rename_at(19, ~'zrotcsv') %>%
  select(Time,X,Y,Z,wquat,xquat,yquat,zquat,xrot,yrot,zrot,Phase,GFO,Movement,xrotcsv,yrotcsv,zrotcsv)
dataLR <- dataLR %>%
  rename_at(17, ~'xrotcsv') %>%
  rename_at(18, ~'yrotcsv') %>%
  rename_at(19, ~'zrotcsv') %>%
...
dataVR <- dataVR %>%
  ...
  rename_at(18, ~'yrotcsv') %>%
  rename_at(19, ~'zrotcsv') %>%
  select(Time,X,Y,Z,wquat,xquat,yquat,zquat,xrot,yrot,zrot,Phase,GFO,Movement,xrotcsv,yrotcsv,zrotcsv,FocusedObject,RayOrigin_x,RayOrigin_y,RayOrigin_z,RayDirection_x,RayDirection_y,RayDirection_z)
...
Pnt <- c(rot_Matrix[1,1]*Pn[1]+rot_Matrix[1,2]*Pn[2]+rot_Matrix[1,3]*Pn[3],rot_Matrix[2,1]*Pn[1]+rot_Matrix[2,2]*Pn[2]+rot_Matrix[2,3]*Pn[3],rot_Matrix[3,1]*Pn[1]+rot_Matrix[3,2]*Pn[2]+rot_Matrix[3,3]*Pn[3]);
  ...
  suppressWarnings(Eye <- as.numeric(c(dataVR$RayDirection_x[c1],dataVR$RayDirection_y[c1],dataVR$RayDirection_z[c1])))
  
  if(!is.na(prod(Eye))){
  
  eyeyaw <- (acos((Eye[1]*N[1]+0+Eye[3]*N[3])/(sqrt(Eye[1]*Eye[1]+Eye[3]*Eye[3])*sqrt(N[1]*N[1]+N[3]*N[3]))))/pi*180;
  eyepitch <- (acos((Eye[1]*Eye[1]+Eye[3]*Eye[3])/(sqrt(Eye[1]*Eye[1]+Eye[2]*Eye[2]+Eye[3]*Eye[3])*sqrt(Eye[1]*Eye[1]+Eye[3]*Eye[3]))))/pi*180;
  
  dataVR$eyepitch[c1] = round(eyepitch,7)
  dataVR$eyeyaw[c1] = round(eyeyaw,7)
  }
...
c1 <- c1 + 1;
  ...
  rm(c0,c1,yaw,pitch,Pnt,Pn,N,R,Rz,Ry,Rx,rot_Matrix,a,b,c,X_n,Y_n,Z_n,xrot,yrot,zrot,Px,Py,Pz,q0,q1,q2,q3,basismatrix1,basismatrix2,Rinv,calquat,calquatinv,C,r00,r01,r02,r10,r11,r12,r20,r21,r22,calw,calx,caly,calz,P_current,P_post,P_pre,postangle,preangle,T_current,T_pre,T_post,V,Eye,eyepitch,eyeyaw)
