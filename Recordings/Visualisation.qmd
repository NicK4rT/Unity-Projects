...
ggplot(data, aes(Gender,, fill = factor(Gender))) +
  ...
  scale_y_continuous(breaks = c(13,18)) +
  theme(axis.line = element_blank(),
        axis.grid = element_blank(),
  plot.title = element_text(hjust=0.5)) +
  labs(fill="Test Subject Gender",
     x=NULL,
     y=NULL,
     title="Gender Identification of Test Subjects")  +
  theme_minimal()
...
geom_bar(position = position_stack(reverse = TRUE),width = 1, stat = "identity") +
    scale_color_hue() +
scale_y_continuous(breaks = c(.5,15.5,30.5),labels = c("1","29","1")) +
theme(axis.line = element_blank(),
  plot.title = element_text(hjust=0.5)) +
  labs(fill="Profession",
     x=NULL,
     y=NULL,
     title="Profession of the Test Subjects") + 
...
theme(axis.line = element_blank(),
  ...
  labs(fill="Degree Type",
     x=NULL,
     y=NULL,
     title="Academic Degree Achieved or Pursued by Test Subjects") + 
  coord_polar(theta = "y", start=0) +
  theme_minimal()


rm(RB.LR.VR,RB.VR.LR)
