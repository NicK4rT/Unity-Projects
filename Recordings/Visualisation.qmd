...
B5.A5C5.time <- subset(B5.A5C5.time, V1<300000 & V1>-300000)
...
B5.A5C5.angle <- subset(B5.A5C5.angle, V1<30 & V1>-30)
...
B5.B6.time %>%
  group_by(V2) %>%
  summarise_at(vars(V1), list(name = mean))
...
Angle  %>%
  ...
  summarise_at(vars(dRB.LR,dRB.VR,dLR.VR), list(name = mean))
...
dataplot1 <- read.csv("US10RB.csv")
dataplot2 <- read.csv("US10LR.csv")
dataplot3 <- read.csv("US10VR.csv")
...
ggplot(dataplot1, aes(x = yaw2, y = pitch2, color = "blue")) +
  ...
  #geom_line(data = as.data.frame(matrix(c(0,30,0,-15.6),nrow=2,ncol=2)),aes(x = V1, y = V2), color = "black") +
    labs(x="Yaw [deg]",
       y="Pitch [deg]",
     title="Movement B5 -> B6 -> B9 -> B8 -> B5 -> B4 -> B1 -> B2 -> B5",
     color = "Task Condition:") +
...
ggplot(dataplot2, aes(x = yaw2, y = pitch2))+#, color = "blue")) +
  geom_path() +
  #geom_path(data = dataplot2, aes(x = yaw2, y = pitch2, color = "green")) +
  #geom_path(data = dataplot3, aes(x = -yaw2, y = -pitch2, color = "red")) +
  #geom_line(data = as.data.frame(matrix(c(0,30,0,-15.6),nrow=2,ncol=2)),aes(x = V1, y = V2), color = "black") +
...
ggplot(dataplot3, aes(x = yaw2, y = pitch2))+#, color = "blue")) +
  geom_path() +
  #geom_path(data = dataplot2, aes(x = yaw2, y = pitch2, color = "green")) +
  #geom_path(data = dataplot3, aes(x = -yaw2, y = -pitch2, color = "red")) +
  #geom_line(data = as.data.frame(matrix(c(0,30,0,-15.6),nrow=2,ncol=2)),aes(x = V1, y = V2), color = "black") +
